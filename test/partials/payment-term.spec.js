...
describe('payment-term', () => {
	...
	describe('annual', () => {
		...
		// Removing in favour of monthly price for the time being
		...
		it('should not have monthly price text by default', () => {
			const name = 'annual';
			const $ = context.template({ options: [{
				name
			}]});
			expect($('.ncf__payment-term__monthly-price').length).to.equal(0);
		});

		it('should have monthly price text if passed', () => {
			const name = 'annual';
			...
			expect($('.ncf__payment-term__monthly-price').length).to.equal(1);
		});
	...
	function testTrial (name) {
		it('should show the correct title copy', () => {
			...
			expect($(TITLE_SELECTOR).text()).to.contain('Trial: Premium Digital');
		});

		it('should not show Trial: Premium Digital for print or bundle trials', () => {
			const $ = context.template({
				options: [{
					name,
					isTrial: true
			...
			expect($(TITLE_SELECTOR).text()).to.not.contain('Trial: Premium Digital');
		});
